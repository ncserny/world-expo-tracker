...
export const FilterControls = ({
  ...
  return <div className="space-y-4 p-4 bg-card border rounded-lg mb-6">
      ...
      <div className="relative">
        ...
        <Input placeholder="Search pavilions..." value={searchTerm} onChange={e => onSearchChange(e.target.value)} className="pl-10" />
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map(category => <Badge key={category} variant={selectedCategories.includes(category) ? 'default' : 'outline'} className="cursor-pointer hover:bg-primary/80 transition-colors" onClick={() => onCategoryToggle(category)}>
              {category}
            </Badge>)}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {availablePavilionCodes.map(code => <Badge key={code} variant={selectedPavilionCodes.includes(code) ? 'default' : 'outline'} className="cursor-pointer hover:bg-primary/80 transition-colors" onClick={() => onPavilionCodeToggle(code)}>
              {code}
            </Badge>)}
        </div>
      </div>
